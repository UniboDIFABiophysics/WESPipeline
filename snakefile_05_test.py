...
for match in match_list:
        ...
        checked_match_list.append(match)
        matches_dict[match] = {}
        matches_dict[match]['T'] = names[0]
        matches_dict[match]['N'] = names[1]
        matches_dict[match]['kit'] = kit_tumor

matches = list(set(checked_match_list))
samples = [s for p in matches for s in p.split('_')]
...
def get_bed_match(wildcards):
    kit = matches_dict[wildcards]['kit']
...
def get_bam(wildcards, sample_type, Dir):
    return (Dir + matches_dict[wildcards][sample_type] + ".bam")

def get_bai(wildcards, sample_type, Dir):
    return (Dir + matches_dict[wildcards][sample_type] + ".bai")

def get_mpileup(wildcards, sample_type, Dir):
    return (Dir + matches_dict[wildcards][sample_type] + ".mpileup")
...
rule all:
  ...
  input:
    ...
    expand(variants_filter + "{match}_all_somatic_annotated_filtered_final.tsv",match=matches),
  message: " THE END "
  run:
    pass
...
#        "{params.adapter_removal} --file1 {input.fastq1} --file2 {input.fastq2}{params.pcr1}{params.pcr2} --stats --trimns --trimqualities --minquality 20 --minlength 80 --output1 {output.fastq1_trimmed} --output2 {output.fastq2_trimmed}"
        "{params.adapter_removal} --file1 {input.fastq1} --file2 {input.fastq2}{params.pcr1}{params.pcr2} --stats --trimns --trimqualities --minquality 20 --minlength 79 --output1 {output.fastq1_trimmed} --output2 {output.fastq2_trimmed}"
...
rule muTect_genome:
    ...
    input:
        ...
        target = lambda wildcards: get_bed_match(wildcards.match),
        normal_bam = lambda wildcards: get_bam(wildcards.match,'N', alignpath_genomebqsr),
        tumour_bam = lambda wildcards: get_bam(wildcards.match,'T', alignpath_genomebqsr),
        normal_bai = lambda wildcards: get_bai(wildcards.match,'N', alignpath_genomebqsr),
        tumour_bai = lambda wildcards: get_bai(wildcards.match,'T', alignpath_genomebqsr),
    output:
        out = mutectpath_genome + "{match}.tsv",
        cov = mutectpath_genome + "{match}_cov_wig.log"
    params:
        dbsnp = dbsnp,
        ref = hg,
    log:
        m = mutectpath_genome_logs + "{match}" + "_mutect.log",
        err = mutectpath_genome_logs + "{match}" + "_mutect_err.log"
    conda:
        pipelinepath + "envs/wes_config_conda_muTect.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_muTect_ref_genome_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : MuTect on genomic alignment"
    shell:
...
rule muTect_MT:
    ...
    input:
        ...
        cosmic_idx = cosmic + '.idx',
        target = MT_bed + ".bed",
        normal_bam = lambda wildcards: get_bam(wildcards.match,'N', alignpath_MTbqsr),
        tumour_bam = lambda wildcards: get_bam(wildcards.match,'T', alignpath_MTbqsr),
        normal_bai = lambda wildcards: get_bai(wildcards.match,'N', alignpath_MTbqsr),
        tumour_bai = lambda wildcards: get_bai(wildcards.match,'T', alignpath_MTbqsr),
    output:
        out = mutectpath_MT + "{match}.tsv",
        cov = mutectpath_MT + "{match}_cov_wig.log"
    params:
        dbsnp = dbsnp,
        ref = MT,
    log:
        m = mutectpath_MT_logs + "{match}" + "_mutect.log",
        err = mutectpath_MT_logs + "{match}" + "_mutect_err.log"
    conda:
        pipelinepath + "envs/wes_config_conda_muTect.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_muTect_ref_MT_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : MuTect on MT alignment"
    shell:
        "java -Xmx2g -jar {input.muTect} --analysis_type MuTect --reference_sequence {params.ref} --cosmic {input.cosmic} --dbsnp {params.dbsnp} --intervals {input.target} --input_file:normal {input.normal_bam} --input_file:tumor {input.tumour_bam} --out {output.out} --coverage_file {output.cov} > {log.m} 2> {log.err}"
...
rule varscan_genome_match:
    """
    This step uses Varscan to call variants.
    """
    input:
        varscan = varscan,
        normal = lambda wildcards: get_mpileup(wildcards.match,'N', mpileup_varscan_genome),
        tumour = lambda wildcards: get_mpileup(wildcards.match,'T', mpileup_varscan_genome),
    output:
        snv = varscanpath_genome + "{match}_snv.tsv",
        indel = varscanpath_genome + "{match}_indel.tsv",
    params:
        ref = hg,
    log:
        varscan_genome_logs + "{match}" + "_varscan.log"
    conda:
        pipelinepath + "envs/wes_config_conda.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_varscan_ref_genome_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : Varscan on match genomic alignment"
    shell:
...
rule varscan_MT_match:
    """
    This step uses Varscan to call variants.
    """
    input:
        varscan = varscan,
        normal = lambda wildcards: get_mpileup(wildcards.match,'N', mpileup_varscan_MT),
        tumour = lambda wildcards: get_mpileup(wildcards.match,'T', mpileup_varscan_MT),
    output:
        snv = varscanpath_MT + "{match}_snv.tsv",
        indel = varscanpath_MT + "{match}_indel.tsv",
    params:
        ref = MT,
    log:
        varscan_MT_logs + "{match}" + "_varscan.log"
    conda:
        pipelinepath + "envs/wes_config_conda.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_varscan_ref_MT_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : Varscan on match MT alignment"
    shell:
        "java -jar {input.varscan} somatic {input.normal} {input.tumour} --output-snp {output.snv} --output-indel {output.indel} --min-avg-qual 15 --strand_filter 1 --min-var-freq 0.05 --somatic-p-value 0.05 2> {log}"
...
rule MergeWriteVariants:
    input:
        mutect_genome = mutectpath_genome + "{match}.tsv",
        mutect_MT = mutectpath_MT + "{match}.tsv",
        snv_genome = varscanpath_genome + "{match}_snv.tsv",
        indel_genome = varscanpath_genome + "{match}_indel.tsv",
        snv_MT = varscanpath_MT + "{match}_snv.tsv",
        indel_MT = varscanpath_MT + "{match}_indel.tsv",
    output:
        m = variants_filter + "{match}_mutect.tsv",
        snp = temp(variants_filter + "{match}_varscan_snv_temp1.tsv"),
        indel = temp(variants_filter + "{match}_varscan_indel_temp1.tsv"),
        v_all = variants_filter + "{match}_varscan.tsv",
    params:
        scripts = scripts,
        name = "{match}",
        outdir = variants_filter,
    message: ">> {wildcards.match} : Merge and Write Variants"
    script:
        "{params.scripts}"+"MergeWriteVariants.py"

rule filterSomatic_SNV:
    input:
        snp = variants_filter + "{match}_varscan_snv_temp1.tsv",
        indel = variants_filter + "{match}_varscan_indel_temp1.tsv",
    output:
        temp(variants_filter + "{match}_varscan_snv_temp2.tsv"),
    params:
        varscan = varscan,
    log:
        variants_filter_logs + "{match}_somaticFilter_snv_err.log"
    conda:
        pipelinepath + "envs/wes_config_conda.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_filterSomatic_SNV_ref_null_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : Varscan Filter Somatic SNV"
    shell:
...
rule filterSomatic_Indel:
    input:
        variants_filter + "{match}_varscan_indel_temp1.tsv",
    output:
        temp(variants_filter + "{match}_varscan_indel_temp2.tsv"),
    params:
        varscan = varscan,
    log:
        variants_filter_logs + "{match}_somaticFilter_indel_err.log"
    conda:
        pipelinepath + "envs/wes_config_conda.yaml"
    benchmark:
        processpath + "benchmarks/benchmark_filterSomatic_Indel_ref_null_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : Varscan Filter Somatic Indel"
    shell:
...
rule KeepOnlyVariants:
    input:
        m = variants_filter + "{match}_mutect.tsv",
        snv = variants_filter + "{match}_varscan_snv_temp2.tsv",
        indel = variants_filter + "{match}_varscan_indel_temp2.tsv",
    output:
        m_tsv = variants_filter +"{match}_mutect_somatic.tsv",
        vsn_tsv = variants_filter +"{match}_varscan_somatic.tsv",
    params:
        scripts = scripts,
        name = "{match}",
        outdir = variants_filter,
    message: ">> {wildcards.match} : Keep Only Variants"
    script:
...
rule MutectSplitVariants:
    input:
        m_tsv = variants_filter +"{match}_mutect_somatic.tsv",
    output:
        ann_genome = variants_filter +"{match}_mutect_annovar_genome.tsv",
        ann_MT = variants_filter +"{match}_mutect_annovar_mt.tsv",
    params:
        scripts = scripts,
        workdir = variants_filter,
        name = "{match}"
    message: ">> {wildcards.match} : Split muTect Variants"
    script:
...
rule annovar_genome_mutect:
    ...
    output:
        variants_filter_logs + "{match}_mutect_genome.hg19_multianno.txt"
    params:
        ...
        humandb = humandb,
        build_ver = build_ver,
        out_label = variants_filter_logs + "{match}_mutect_genome",
    log:
        variants_filter_logs + "{match}_mutect_genome_annovar_err.log"
    benchmark:
        processpath + "benchmarks/benchmark_annovar_muTect_ref_genome_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : annotating with ANNOVAR muTect genome variants"
    shell:
...
rule annovar_MT_mutect:
    input:
        ...
        annovar_dbs = annovar_dbs,
    output:
        variants_filter_logs + "{match}_mutect_mt_g.GRCh37_MT_multianno.txt"
    params:
        ...
        out_label = variants_filter_logs + "{match}_mutect_mt_g",
    log:
        variants_filter_logs + "{match}_mutect_mt_g_annovar_err.log"
    benchmark:
        processpath + "benchmarks/benchmark_annovar_muTect_ref_MT_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : annotating with ANNOVAR muTect MT variants"
    shell:
...
rule VarscanSplitVariants:
    input:
        vsn_tsv = variants_filter +"{match}_varscan_somatic.tsv",
    output:
        ann_genome = variants_filter +"{match}_varscan_annovar_genome.tsv",
        ann_MT = variants_filter +"{match}_varscan_annovar_mt.tsv",
    params:
        scripts = scripts,
        workdir = variants_filter,
        name = "{match}"
    message: ">> {wildcards.match} : Split Varscan Variants"
    script:
...
rule annovar_genome_varscan:
    input:
        ...
        annovar_dbs = annovar_dbs,
    output:
        variants_filter_logs + "{match}_varscan_genome.hg19_multianno.txt"
    params:
        protocol = config['protocols']['genome'],
        operation = config['operations']['genome'],
        tableannovar = tableannovar,
        humandb = humandb,
        build_ver = build_ver,
        out_label = variants_filter_logs + "{match}_varscan_genome",
    log:
        variants_filter_logs + "{match}_varscan_genome_annovar_err.log"
    benchmark:
        processpath + "benchmarks/benchmark_annovar_varscan_ref_genome_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : annotating with ANNOVAR varscan genome variants"
    shell:
        "perl {params.tableannovar} {input.ann_genome} {params.humandb} -buildver {params.build_ver} -out {params.out_label} -remove -protocol {params.protocol} -operation {params.operation} 2> {log}"
...
rule annovar_MT_varscan:
    input:
        ...
        annovar_dbs = annovar_dbs,
    output:
        variants_filter_logs + "{match}_varscan_mt_g.GRCh37_MT_multianno.txt"
    params:
        protocol = config['protocols']['MT'],
        operation = config['operations']['MT'],
        tableannovar = tableannovar,
        humandb = humandb,
        mitochondrial_ver = mitochondrial_ver,
        out_label = variants_filter_logs + "{match}_varscan_mt_g",
    log:
        variants_filter_logs + "{match}_varscan_mt_g_annovar_err.log"
    benchmark:
        processpath + "benchmarks/benchmark_annovar_varscan_ref_MT_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : annotating with ANNOVAR varscan MT variants"
    shell:
        "perl {params.tableannovar} {input.ann_MT} {params.humandb} -buildver {params.mitochondrial_ver} -out {params.out_label} -remove -protocol {params.protocol} -operation {params.operation} 2> {log}"
...
rule MergeMutectVarscan:
    input:
        m_gen = variants_filter_logs + "{match}_mutect_genome.hg19_multianno.txt",
        m_mt = variants_filter_logs + "{match}_mutect_mt_g.GRCh37_MT_multianno.txt",
        vsn_gen = variants_filter_logs + "{match}_varscan_genome.hg19_multianno.txt",
        vsn_mt = variants_filter_logs + "{match}_varscan_mt_g.GRCh37_MT_multianno.txt",
        m_tsv = variants_filter + "{match}_mutect_somatic.tsv",
        vsn_tsv = variants_filter + "{match}_varscan_somatic.tsv",
    output:
        table_out = variants_filter + "{match}_all_somatic_annotated.tsv",
    params:
        scripts = scripts,
        workdir = variants_filter_logs,
        name = "{match}",
    message: ">> {wildcards.match} : merging muTect and Varscan Variants"
    script:
...
rule create_rsID_table:
    input:
        tsv = variants_filter + "{match}_all_somatic_annotated.tsv",
        maf = "/",
    output:
        out = temp(variants_filter + "{match}_rsID.tsv"),
    params:
        scripts = scripts,
    message: ">> {wildcards.match} : create rsID table"
    script:
        "{params.scripts}" + "fEP_script.py"

rule filterExonicPolymorphic:
    input:
        variants_filter + "{match}_rsID.tsv",
    output:
        temp("{match}_rsID_maf.tsv")
    params:
        rsIDfilter = rsIDfilter,
    log:
        info = variants_filter_logs + "{match}_rsID_session_info.log",
        err_1 = variants_filter_logs + "{match}_rsID_err_script.log",
        err_2 = variants_filter_logs + "{match}_rsID_err.log",
    conda:
        pipelinepath + "envs/wes_config_conda_R.yaml",
    benchmark:
        processpath + "benchmarks/benchmark_fEP_ref_null_subject_{match}" + "_n_sim_{n_sim}_cputype_{cpu_type}_Totthrs_{thrs}_Rulethrs_1_ncpu_{n_cpu}.txt".format(n_sim=n_sim, cpu_type=cpu_type, thrs=thrs, n_cpu=n_cpu)
    message: ">> {wildcards.match} : filtering exonic/non-synonymous/non-polymorphic"
    shell:
...
rule checkMAFs:
    input:
        tsv = variants_filter + "{match}_all_somatic_annotated.tsv",
        maf = "{match}_rsID_maf.tsv",
    output:
        out = variants_filter + "{match}_all_somatic_annotated_filtered.tsv",
    params:
        scripts = scripts,
    message: ">> {wildcards.match} : checking MAFs"
    script:
        "{params.scripts}"+"fEP_script.py"
...
rule Remove_segdups:
    input:
        segdups = variants_filter + "{match}_all_somatic_annotated_filtered.tsv",
    output:
        no_segdups = variants_filter + "{match}_all_somatic_annotated_filtered_final.tsv",
    message: ">> {wildcards.match} : Removing segdups variants"
    run:
        import pandas as pd

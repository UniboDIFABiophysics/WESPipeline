...
# python script paths
checkMpileupSize = pipelinepath + config['scripts']['checkMpileupSize']
downloadDecompressMergeFastq = pipelinepath + config['scripts']['downloadDecompressMergeFastq']
computeOverallBAMstats = pipelinepath + config['scripts']['computeOverallBAMstats']
mutectPreAnnovar = pipelinepath + config['scripts']['mutectPreAnnovar']
varscanSomaticFilterPreAnnovar = pipelinepath + config['scripts']['varscanSomaticFilterPreAnnovar']
mergeAnnotateLabelFilter = pipelinepath + config['scripts']['mergeAnnotateLabelFilter']
editBEDChromField = pipelinepath + config['scripts']['editBEDChromField_02']
...
rule downloadDecompressMergeFastq:
    ...
    params:
        ...
        downloadDecompressMergeFastq = downloadDecompressMergeFastq,
    resources:
        disk = 1
    benchmark:
    ...
    script:
        "{params.downloadDecompressMergeFastq}"
...
rule overall_bam_stats:
    ...
    params:
        computeOverallBAMstats = computeOverallBAMstats,
    message: '>> {wildcards.sample} : Computing and merging overall BAM statistics'
    script:
        '{params.computeOverallBAMstats}'
...
rule mutect_preAnnovar:
    ...
    params:
        mutectPreAnnovar = mutectPreAnnovar,
        pair = '{pair}',
    message: '>> {wildcards.pair} : pre-process of Mutect for ANNOVAR'
    script:
        '{params.mutectPreAnnovar}'
...
rule varscan_somaticFilter_preAnnovar:
    ...
    params:
        varscanSomaticFilterPreAnnovar = varscanSomaticFilterPreAnnovar,
        varscan = varscan,
    ...
    script:
        '{params.varscanSomaticFilterPreAnnovar}'
...
rule merge_annotate_label_filter:
    ...
    params:
        mergeAnnotateLabelFilter = mergeAnnotateLabelFilter,
    ...
    script:
        '{params.mergeAnnotateLabelFilter}'
